...
namespace monocon {
    ...
    //
    // Runs raw int8 in, raw int8 out. Quantization (preprocess) and
    // dequantization (postprocess) deliberately live OUTSIDE this class,
    // in preprocess/quantize.* and postprocess/dequantize.* respectively —
    // this class's only job is "talk to the AIPU."
    //
    ...
    class AipuInference {
    public:
        AipuInference(const std::string& model_dir, int num_cores = 4);
        ~AipuInference();
        ...
        // Raw int8 inference: caller must have already quantized+padded
        // the input into a buffer matching input_info(0).size_bytes,
        // and will read raw int8 output from output_info(0) afterward.
        //
        // Returns pointers to internal buffers — valid until the next run().
        int8_t* input_buffer(int index = 0);
        const int8_t* run();  // runs, returns pointer to output_info(0)'s raw buffer
